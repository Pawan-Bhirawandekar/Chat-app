...
const LoginPage = () => {
  ...
  const onSubmitHandler =(event) =>{
    ...
    login(currState === "Sign Up" ? 'singup' : 'login', {fullName, email, password, bio})
  }
        ...
        <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
          ...
          <div className='flex flex-col gap-2'>
            {currState ==="Sign Up" ? (
              ...
              <p className='text-sm text-gray-400'>Creat an Account <span onClick={()=>setCurrState("Sign Up")} className='font-medium text-violet-500 cursor-pointer'>Click here</span></p>
            )}
          </div>
        </form>
    </div>
